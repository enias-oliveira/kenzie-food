...
class Command(BaseCommand):
        ...
        with open(csvfile) as file:
            ...
            for row in csv_data:
                author = Author.objects.get_or_create(
                    id=row['AuthorId'],
                    name=row['AuthorName'])[0]
                category = Category.objects.get_or_create(category=row['RecipeCategory'])[0]
                ...
                recipe_obj_list.append(Recipe(
                    ...
                    recipe_servings=int(float(row['RecipeServings'])) if row['RecipeServings'] else None,
                ...
                keywords = row['Keywords'].replace('c(', '').replace(')', '').replace('"', "").split(', ')
            ...
            Recipe.objects.bulk_create(recipe_obj_list)
    ...
    def handle(self, *args, **kwargs):
        ...
        recipe_files = [ os.path.join(BASE_DIR, "data/recipes/")+file for file in os.listdir(os.path.join(BASE_DIR, "data/recipes")) ]
